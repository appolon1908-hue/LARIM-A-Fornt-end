...
describe('LARIMÍA operations web surface', () => {
  const requiredPages = [
    'index.vue',
    'dispatch.vue',
    'providers.vue',
  ...
  it('renders the Nuxt page outlet and links every primary operator area', () => {
    const shell = read('app/app.vue')

    expect(shell).toContain('<NuxtPage')
    for (const route of ['/', '/dispatch', '/providers', '/safety', '/finance', '/catalog', '/partners']) {

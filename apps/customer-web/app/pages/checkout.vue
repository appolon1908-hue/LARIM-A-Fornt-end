...
const api=useLarimiaApi();const flow=useBookingFlow();const busy=ref(false);const error=ref('')
async function confirm(){
  ...
  try{
    ...
    flow.value.booking=await api.confirmBooking(booking.id,booking.version,{idempotencyKey:crypto.randomUUID()})
